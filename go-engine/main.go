...
func main() {
	...
	tlsSkip           := flag.Bool("tls-skip", false, "Skip TLS certificate verification")
	recursive         := flag.Bool("recursive", false, "Recursively scan found directories")
	depth             := flag.Int("depth", 3, "Max recursion depth (default 3)")
	maxRecursePerLevel := flag.Int("max-recurse", 30, "Max directories to recurse per level (0 = unlimited)")

	flag.Parse()

	if *mode == "" || *target == "" || *wordlist == "" {
		fmt.Fprintln(os.Stderr, "usage: wlrecon-engine -mode <dir|endpoint|user|email> -target <url> -wordlist <path> [-threads N] [-timeout N] [-proxy url] [-match-string str] [-invalid-string str] [-headers 'K:V,...'] [-params 'k=v&...'] [-tls-skip] [-recursive] [-depth N] [-max-recurse N]")
		os.Exit(1)
	}
	...
	cfg := EngineConfig{
		...
		Recursive:          *recursive,
		MaxDepth:           *depth,
		MaxRecursePerLevel: *maxRecursePerLevel,
	}
